...
Deno.serve(async (req) => {
  ...
  try {
    for (const { msg_id, message: request } of messages) {
      ...
      const backendResponse = await fetch(`${backendApiUrl}/api/quizzes`, {
        ...
        body: JSON.stringify({
          ...
          types_description: request.types.map(
      ...
      quizElements.forEach((quizElement, i) => {
        for (const quizResult of quizResults) {
          quizElementScores.push({
            ...
            score:
              response.quiz_elements[i].type_weights[quizResult.title] || 0,
          });
        }
      });
      console.log(quizElementScores);
      const { data, error } = await supabase
